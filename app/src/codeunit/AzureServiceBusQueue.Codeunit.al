codeunit 82561 "BC2ASB Azure Service Bus Queue"
{
    var
    ...
    begin
        if not QueueSetup.Get() then
            Error(QueueNotEnabledErr);

        if not QueueSetup.IsEnabled then
            Error(QueueNotEnabledErr);

        Resource := StrSubstNo('https://%1.servicebus.windows.net/%2',
            QueueSetup.ServiceBusNamespace,
            QueueSetup.QueueName);

        SharedKeyName := QueueSetup.KeyName;
        SharedKey := QueueSetup.GetSharedAccessKey();
